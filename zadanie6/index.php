<?php
    ...
    try
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET')
        {
            ...
            if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login']) && $_SESSION['login'] == 'Admin')
            {
                $user = 'u52864';
                $password = '3567354';
                ...
                $messages[] = sprintf('<div style = "margin-left: 30px;">Вы редактируете данные как Администратор:</div>');
            }

            else if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login']))
            {
                $user = 'u52864';
                $password = '3567354';
                $database = new PDO('mysql:host=localhost;dbname=u52864', $user, $password, [PDO::ATTR_PERSISTENT => true, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
