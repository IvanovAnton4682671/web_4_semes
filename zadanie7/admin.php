                ...
                <div class = "signup-form" style = "width: 65%;">
                    ...
                    ?>
                        <table>
                            ...
                            echo '<p>↓ ↓ ↓</p>';
                            $result1 = $database -> query("SELECT * FROM Connection WHERE ID_Superpower = 1");
                            $result2 = $database -> query("SELECT * FROM Connection WHERE ID_Superpower = 2");
                            $result3 = $database -> query("SELECT * FROM Connection WHERE ID_Superpower = 3");
                            $result4 = $database -> query("SELECT * FROM Connection");
                            $row1 = 0;
                            $row2 = 0;
                            $row3 = 0;
                            $row4 = 0;
                            while ($roww = $result1 -> fetch()) { if ($roww['ID_Superpower'] == 1) $row1++; }
                            while ($roww = $result2 -> fetch()) { if ($roww['ID_Superpower'] == 2) $row2++; }
                            while ($roww = $result3 -> fetch()) { if ($roww['ID_Superpower'] == 3) $row3++; }
